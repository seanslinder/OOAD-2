...
PizzaPtr buildPizza(const std::unordered_map<std::string, std::string>& params) {
    PizzaPtr pizza;

    const auto baseIt = params.find("base");
    const std::string base = (baseIt == params.end()) ? "italian" : baseIt->second;

    if (base == "bulgarian") {
        pizza = std::make_shared<BulgarianPizza>();
    } else {
        pizza = std::make_shared<ItalianPizza>();
    }

    if (isEnabled(params, "tomato")) {
        pizza = std::make_shared<TomatoTopping>(pizza);
    }
    if (isEnabled(params, "cheese")) {
        pizza = std::make_shared<CheeseTopping>(pizza);
    }
    if (isEnabled(params, "olives")) {
        pizza = std::make_shared<OlivesTopping>(pizza);
    }
    if (isEnabled(params, "mushrooms")) {
        pizza = std::make_shared<MushroomsTopping>(pizza);
    }
...
std::string responseForRequest(const std::string& method, const std::string& target) {
    ...
    const PizzaPtr pizza = buildPizza(params);

    std::ostringstream body;
    body << "{"
         << "\"name\":\"" << escapeJson(pizza->name()) << "\","
         << "\"price\":" << pizza->cost() << ","
         << "\"base\":\"" << ((params.count("base") && params.at("base") == "bulgarian") ? "bulgarian" : "italian") << "\""
         << "}";
...
int main() {
    ...
    std::cout << "C++ pizza backend started: http://localhost:" << port << "\n";
